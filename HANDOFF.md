...
| AI 分析 | `analyze.py` | Claude Haiku（dev）/ Sonnet（prod），輸出含 top5_news + investor_reminder |
...
- 所有 Phase 1 + Phase 2 程式碼已推上去
- `.env` 已在 `.gitignore` 保護

### Phase 3 — Remotion 影片（大幅進展）
...
| 新聞條目場景 | ✅ | 圖片（55%）+ 標題/摘要，**第一篇/第二篇**標籤 |
| 綜合洞察場景 | ✅ | 觀點分析（藍框）+ 投資人提醒（金框）+ **背景圖** |
| Ending 場景 | ✅ | 品牌結尾 + hashtag + 免責聲明 + **背景圖** |
...
| 旁白整合 | ✅ | `<Audio>` 串入每個 Remotion 場景，**場景時長動態跟著音檔走**（+2 秒 buffer） |
...
| **UI 可讀性強化** | ✅ | `--text-2` #7a7a7a → #b0b0b0、`--text-3` #3d3d3d → #707070；月份標籤、日期、標籤全面提升對比 |
| **佈局修正** | ✅ | 移除 `home-main` 全域 `max-width:760px`；新聞頁籤用 `.news-inner` 限寬，儀表板/影片頁籤可全寬（TradingView 圖表不再被壓縮）|
| **影片頁籤** | ✅ | 改為 YouTube 頻道嵌入：頻道 Hero 卡片 + 16:9 iframe + 訂閱按鈕；需填 `YOUR_CHANNEL_ID` |
| **頁尾** | ✅ | YouTube / Instagram / Threads 社群圖示（hover 色彩特效）+ 著作權聲明「著作權歸原作者所有，未經授權請勿轉載或商業使用」|
| FAB AI 對話 | ✅ | 右下角雷達按鈕開啟側邊抽屜，串接 Claude Haiku API |
| **AI 對話改善** | ✅ | 放寬系統提示（允許財經術語說明）；新增歡迎訊息說明用途；6 個範例問題（重點解析、雙軌分歧、技術位置、情緒判斷、術語說明、風險提示）|
| 多頁 Vite | ✅ | `vite.config.js` 設定三入口（main/article/dashboard）+ dev `/api/chat` 中介層 |
| Vercel API | ✅ | `web/api/chat.js`（一般文章問答）、`web/api/dashboard-chat.js`（儀表板 AI）、`web/api/social-stats.js`（社群統計）|
...
| 儀表板 AI 聊天 | ✅ | 可詢問流量趨勢、內容策略、平台比較；5 個範例快捷問題；無 RAG 限制（自由討論社群策略）|
| 社群統計 API | ✅ | `web/api/social-stats.js` — YouTube（即時）、Instagram（需 Token）、Threads（需 Token）、YouTube 留言抓取 |
...
1. **網站社群連結填入** ⚠️
   - `web/index.html` 搜尋 `YOUR_CHANNEL_HERE`、`YOUR_CHANNEL_ID`、`YOUR_IG_HERE`、`YOUR_THREADS_HERE`，替換為實際帳號
   - Vercel 環境變數需新增 `YOUTUBE_CHANNEL_ID`

2. **個人儀表板密碼** ⚠️
   - 密碼設定在 `web/js/dashboard.js` 第 10 行 `CORRECT_PASS`，上線前請確認已修改

3. **Instagram / Threads API 設定** ⚠️
...
4. **Oracle Cloud VM 部署** ✅（已部署，IP: `129.150.39.175`）
...
6. **Vercel 部署前端** ⚠️
   ...
   - vite.config.js 已加入第三入口（`dashboard: d.html`）

### 中期（Phase 3 剩餘）

7. **Discord Bot `/run` 控制面板測試**
   - 確認 Bot 重啟後持久化按鈕（`_ControlPanelView`）仍可用
   - 確認 `step_render_video()` 在 `main.py` 已實作（影片發布按鈕依賴此函式）

8. **Nano Banana 圖片生成**（待開 Billing）
...
10. **場景時長微調**（依旁白實際長度）
    - 若有靜音問題，調整 `video/src/RadarVideo.tsx` 的 `BUFFER_FRAMES` 常數
...
```
├── main.py              # 主管線（7 步驟 + Step 3.5 新聞生成 + Step 4.5 圖片/旁白）
...
│   ├── src/
│   │   ├── RadarVideo.tsx       # 場景時長動態計算（durations.json）
│   │   ├── Root.tsx             # Composition 總時長動態計算
│   │   └── data/
│   │       ├── sample.json      # 每日自動更新
│   │       └── durations.json   # 每日音檔時長（generate_audio.py 生成）
│   └── public/
│       ├── audio/               # 8 個 MP3（男聲）
│       └── *.jpg                # Pexels 圖片
├── web/                 # 前端展示網頁（Phase 4）
│   ├── index.html       # 首頁：文章列表（三頁籤）
│   ├── article.html     # 文章頁：讀取 ?date=YYYYMMDD（含 AI 聊天抽屜）
│   ├── d.html           # 個人儀表板（/d，密碼保護）
│   ├── vite.config.js   # 多頁設定（3 入口：main/article/dashboard）
│   ├── vercel.json      # Vercel 部署設定（含 /d 路由 + 3 個 serverless functions）
│   ├── css/style.css    # 極簡深色主題（7 token）
...
│   ├── api/dashboard-chat.js  # 儀表板 AI（社群策略分析 + 文字探勘）
